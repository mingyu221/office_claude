...
## 6-b. 표기 규약 (2026-09-02 결정)

기존 컬럼명 `dchg` / `dvol` 은 **읽는 사람이 무엇의 약자인지 알 수 없다**는 문제가
실제로 발생했다. `d` = delta, `chg` = charge 였다.

**기존 산출물(`analysis_288.csv`, `km_merge_*.csv`, `figs/*`)은 그대로 둔다.**
재생성 비용보다 이름이 바뀐 파일이 섞이는 혼란이 크다.

**앞으로 새로 만드는 표·그림·컬럼은 아래 표기를 쓴다.**

| 기존 | 새 컬럼명 | 발표·그림 표기 | 뜻 |
|---|---|---|---|
| `dchg` | `delta_charge` | **Δq** | 변이로 생긴 형식전하 변화 |
| `dvol` | `delta_volume` | **ΔV** | 잔기 부피 변화 (Å³) |
| `chg` / `chg0` | `charge_mut` / `charge_wt` | — | 치환 / 원래 잔기의 형식전하 |

Δq 는 전하 변화의 관례적 표기라 별도 설명 없이 읽힌다.

**약어가 들어간 표나 그림을 산출할 때는 변수 사전을 함께 낼 것.** 이 문서 하단과
보고서 페이지 부록에 전체 사전이 있다.
