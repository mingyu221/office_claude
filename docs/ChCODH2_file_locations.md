...
> **2026-09-18 — 3kji 검증 완료 (CELL S0)**
> 디스크의 `3kji.pdb` 는 **정상 L 구조**다(손대칭 `+400/-0`). 거울상이 아니다.
> REMARK 350 조립체 2개는 **둘 다 정상 회전(det = +1)이고 둘 다 4-Cys 자리를 만든다**
> — 서로 다른 조립체가 아니라 비대칭단위의 **같은 이량체 사본 두 개**다.
> 조립체 1(A+B) 배위 2.23–2.28 Å · 조립체 2(B+C) 2.02–2.13 Å. 자리의 금속은 **ZN**
> (결정화 중 들어간 것이고, 이 자리의 Ni 결합은 별도 실험으로 증명돼 있다).
> 따라서 앞서 보고한 **B–B2 = 2.13 Å 은 유효한 값**이다.
> S0 이 두 사본 모두 템플릿을 만든다 — `cooc1_merged_asm1.pdb` / `asm2.pdb`.
