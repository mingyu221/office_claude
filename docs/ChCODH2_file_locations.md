...
> **⚠ 2026-09-18 — 3kji 좌표 출처 미확정 (워크로그 11부)**
> `3kji` 이름의 좌표 파일이 두 종류고 하나는 **거울상**이다. 위 표에서 `3kji.pdb`
> 를 쓴 세 줄(원 질의 · 이량체 질의 · 단일사슬 합침)은 **어느 파일에서 나온
> 기하인지 확정되지 않았다.** 거울상은 원자 간 거리를 전부 보존하므로
> RMSD·거리 검사로는 안 걸린다.
> 민감도 노트북의 **CELL S0** 이 (1) 파일 손대칭 (2) BIOMT 연산자의 `det`
> (3) 파일 안 금속에서 3.2 Å 안의 Cys SG 개수 로 옳은 파일·조립체를 고른다.
> S0 을 돌려 `COOC1_PDB` 를 확정하고 `cooc1_ni_site_merged.pdb` 를 다시 만든
> 뒤에야 위 세 줄이 유효하다.
> Foldseek·mmseqs·Boltz 결과는 3kji 좌표를 질의 기하로 쓰지 않으므로 영향 없다.
